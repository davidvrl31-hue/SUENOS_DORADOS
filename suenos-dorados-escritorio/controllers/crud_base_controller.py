...
class BaseCrudController:
    ...
    def save_record(self, view, dialog=None):
        ...
        try:
            ...
            # Auto-generar SKU si es variante nueva y viene vacío o sin valor
            if is_new and config["table"] == "variantes_producto":
                if not values.get("sku"):
                    values["sku"] = "SKU-" + uuid4().hex[:8].upper()
...
def _to_camel(data: dict) -> dict:
    """Convierte claves de snake_case a camelCase y normaliza valores para JSON."""
    return {_snake_to_camel(k): _serialize_value(v) for k, v in data.items()}
