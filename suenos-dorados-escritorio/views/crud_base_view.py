...
class BaseCrudView(ft.Container):
    ...
    def _build_field_control(self, field_name, field_type):
        ...
        if field_name == "sku" and self.current_config.get("table") == "variantes_producto" and self.selected_record_id is None:
            # SKU se genera automáticamente al crear; mostrar hint informativo
            return ft.TextField(
                label="SKU",
                hint_text="Se generará automáticamente",
                width=390,
                dense=True,
                read_only=True,
    ...
    def _store_image_file(self, source_path):
        ...
        # Subir a la API → retorna URL pública accesible desde móvil y web
        try:
            import api_client
            return api_client.upload_imagen(str(source))
        except Exception as exc:
            # Fallback: copia local si la API no está disponible
            ...
            raise RuntimeError(
                f"No se pudo subir la imagen a la API ({exc}). "
                f"Se guardó localmente: solo visible en este equipo."
            )
    ...
    def _open_record_dialog(self, record=None):
        ...
        if record is None:
            ...
            # Auto-generar SKU visible si la tabla lo requiere
            if self.current_config.get("table") == "variantes_producto" and "sku" in self.form_controls:
                sku_control = self.form_controls["sku"]
                generated_sku = "SKU-" + uuid4().hex[:8].upper()
                sku_control.value = generated_sku
                sku_control.hint_text = "SKU generado automáticamente"
        else:
